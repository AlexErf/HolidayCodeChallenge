
#include <iostream>
#include "days/days.h"

int main(int argc, char ** argv) {

	std::cout << "Day 1 Part 1: " << day1p1() << std::endl;
	std::cout << "Day 1 Part 2: " << day1p2() << std::endl;
	std::cout << "Day 2 Part 1: " << day2p1() << std::endl;
	std::cout << "Day 2 Part 2: " << day2p2() << std::endl;
	std::cout << "Day 3 Part 1: " << day3p1() << std::endl;
	std::cout << "Day 3 Part 2: " << day3p2() << std::endl;
	std::cout << "Day 4 Part 1: " << day4p1() << std::endl;
	std::cout << "Day 4 Part 2: " << day4p2() << std::endl;
	std::cout << "Day 5 Part 1: " << day5p1() << std::endl;
	std::cout << "Day 5 Part 2: " << day5p2() << std::endl;
	std::cout << "Day 6 Part 1: " << day6p1() << std::endl;
	std::cout << "Day 6 Part 2: " << day6p2() << std::endl;
	std::cout << "Day 7 Part 1: " << day7p1() << std::endl;
	std::cout << "Day 7 Part 2: " << day7p2() << std::endl;
	return 0;

}


